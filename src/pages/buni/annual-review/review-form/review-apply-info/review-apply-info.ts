...
/**
 * Generated class for the ReviewApplyInfoPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-review-apply-info',
  templateUrl: 'review-apply-info.html',
})
export class ReviewApplyInfoPage {
  cache: any;//缓存
  sqrxm: string;//申请人姓名
  zjlb: string;//证件类别
  zjhm: string;//证件号码
  ...
  ionViewDidLoad() {
    ...
    this.cache = this.local.read('review-applyInfoCache');
    if (this.cache) {
      if (this.cache.zjhm == this.userInfo.crdt_No) {
        ...
        this.jzjwhList = this.cache.jzjwhLis;
  ...
  getSqrxx() {
    ...
    this.httpService.httpPost(10, url, body).then(response => {
      if (response.code == 0) {
        ...
        this.selectedhjdcq = response.sqxx.hjszcq;
        let hjarea = this.hjcqList.filter(x => { return (x.cqid.toString() == this.selectedhjdcq) });
        this.hjjdList = hjarea[0]['chObj'];
        this.selectedhjdjd = response.sqxx.hjszjd;
        this.code = response.sqxx.code;
        if (!this.cache) {
          this.gethjjwhList();
        }
        this.selectedhjdjwh = response.sqxx.hjssjwh;
        this.hjxxdz = response.sqxx.sqrhjddz;
        this.selectedjzdcq = response.sqxx.jzdszcq;
        let jdarea = this.jzcqList.filter(x => { return (x.cqid.toString() == this.selectedjzdcq) });
        this.jzjdList = jdarea[0]['chObj'];
        this.selectedjzdjd = response.sqxx.jzdszjd;
        if (!this.cache) {
          this.getjzjwhList();
        }
        this.selectedjzdjwh = response.sqxx.jzdssjwh;
        this.jzxxdz = response.sqxx.jzdz;
        this.call = response.sqxx.gddh;
        this.phone = response.sqxx.yddh;
  ...
  cacheData() {
    ...
    this.local.write('review-applyInfoCache', cache);
  }
  ...
  saveSqrxx(wblsh, jtlb, sqcq, sqjd, sqrxm, sqrzjlb, sqrzjhm, sqrhjdcq, sqrhjdjd,
    ...
    let ele = "bpCategoryCode=web233052" + "&sqrxxDTO.id=" + "&sqrxxDTO.wblsh=" + wblsh + "&sqrxxDTO.jtlb=" + jtlb + "&sqrxxDTO.code=" + this.code + "&sqrxxDTO.sqcq=" + sqcq +
    ...
    this.httpService.httpPost(8, url, ele).then(response => {
      if (response.success == true) {
        this.cacheData();
        this.navCtrl.push("ReviewFamilyInfoPage", { wwsqbh: wblsh });
      } else {
  ...
  save() {
    ...
    if (!this.selectedhjdcq) {
      ...
      this.comm.showConfirm("CCB建融家园公租房","确认是否保存？", () => {
        ...
        this.jtlb = this.local.read("review-homeStyle")[0].key;
