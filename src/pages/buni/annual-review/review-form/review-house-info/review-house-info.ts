...
import { AddReviewHouseInfoPage } from './add-review-house-info';
import { NativeService } from '../../../../../providers/NativeService';
import { StorageService } from '../../../../../providers/localstorage';

@IonicPage()
@Component({
  selector: 'page-review-house-info',
  templateUrl: 'review-house-info.html',
})
export class ReviewHouseInfoPage {

  private zfqkJson: Array<HouseInfo> = []; // 住房情况信息
  ...
  ionViewDidLoad() {
    let cache: any = this.storage.read('review-houseInfoCache');
    if (cache) {
  ...
  addHouseInfo() {
    let profileModal = this.modalCtrl.create(AddReviewHouseInfoPage, { houseTypes: this.houseTypes });
  ...
  //编辑住房信息
  editHouseInfo(item) {
    let profileModal = this.modalCtrl.create(AddReviewHouseInfoPage, { item: item, houseTypes: this.houseTypes });
    profileModal.onDidDismiss(data => {
  ...
  save() {
    this.comm.showConfirm("CCB建融家园公租房","确认是否保存？", () => {
      ...
      let body = "bpCategoryCode=web233052&wwsqbh=" + this.storage.read('timestamp') +
      ...
      this.httpService.httpPost(8, url, body).then(response => {
        if (response.status == 0) {
          ...
          this.cacheData();
          this.navCtrl.push("ReviewPropertyInfoPage");
        } else {
  ...
  cacheData() {
    ...
    this.storage.write('review-houseInfoCache', cache);
  }
}
