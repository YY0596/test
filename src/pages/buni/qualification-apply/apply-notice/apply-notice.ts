...
@IonicPage()
@Component({
  selector: 'page-apply-notice',
  templateUrl: 'apply-notice.html',
})
export class ApplyNoticePage {
  ...
  ionViewDidLoad() {
    ...
    this.nextPageInfo = nextPageInfo.filter(x => { return x['lx'] != 1 });
  ...
  nextPage() {
    if (this.hasRead) {
      this.navCtrl.push("ApplyCommitmentPage", { nextPageInfo: this.nextPageInfo });
    }
  }
}
