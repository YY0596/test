...
@IonicPage()
@Component({
  selector: 'page-apply-style',
  templateUrl: 'apply-style.html',
})
export class ApplyStylePage {
  ...
  changeArea() {
    ...
    let body = "szcq=" + this.selectedArea + "&ids=1,2,3,4,5";
    ...
    this.httpService.httpPost(9, url, body).then(response => {
      if (response.code == 0) {
        response.data.forEach(element => {
          ...
          } else if (element['lx'] == 3) {
            this.homeStyleNotice = element['nr'];
          } else if (element['lx'] == 5) {
            this.storage.write('qualificationApply-imgInfoNotice', element['nr']);// 缓存上传图片信息注意事项
          }
        });
        this.nextPageInfo = response.data.filter(x => { return (x['lx'] != 2 && x['lx'] != 3) });
      } else {
  ...
  nextPage() {
    ...
    this.storage.write('qualificationApply-homeStyle', homeStyle);
    this.storage.write('qualificationApply-szcq', this.selectedArea);
    this.navCtrl.push("ApplyNoticePage", { nextPageInfo: this.nextPageInfo });
  }
}
