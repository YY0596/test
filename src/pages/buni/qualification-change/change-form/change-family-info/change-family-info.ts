...
import { AddChangeFamilyInfoPage } from './add-family-info/add-change-family-info';
...
@IonicPage()
@Component({
  selector: 'page-change-family-info',
  templateUrl: 'change-family-info.html',
})
export class ChangeFamilyInfoPage {
  ...
  ionViewDidLoad() {
    let cache: any = this.storage.read('change-familyInfoCache');
    if (cache) {
  ...
  back = (e: UIEvent) => {
    ...
    this.storage.write('change-familyInfoCache', cache);
    this.navCtrl.pop();
  }

  addFamilyInfo() {
    let profileModal = this.modalCtrl.create(AddChangeFamilyInfoPage, { rygxList: this.rygxList });
  ...
  getJtcyxx() {
    ...
    let url = "getJtcyxxOfJtxxbg.action";
    let body = "zjlx=1010" + "&zjhm=" + this.userInfo.crdt_No;
      ...
      if (response.code == 0) {
        response.data.forEach(element => {
          ...
          if (element.jtry_isfgby == "0") {
            ...
            familyInfo.jtry_fbykssj = element.jtry_fbykssj;
            familyInfo.jtry_fbykssj_text = element.jtry_fbykssj
            familyInfo.jtry_fbyjssj = element.jtry_fbyjssj;
            familyInfo.jtry_fbyjssj_text = element.jtry_fbyjssj;
          }
  ...
  //编辑资产信息
  editFamilyInfo(item) {
    let profileModal = this.modalCtrl.create(AddChangeFamilyInfoPage, { item: item, rygxList: this.rygxList });
    profileModal.onDidDismiss(data => {
      if (!data) {
        return;
      }
  ...
  save() {
    ...
    this.comm.showConfirm("CCB建融家园公租房","确认是否保存？", () => {
      ...
      let body = "bpCategoryCode=web233051&wwsqbh=" + this.local.read('timestamp') +
        "&jtryJson=" + JSON.stringify(this.jtryJson);
      this.httpService.httpPost(8, url, body).then(response => {
        if (response.status == 0) {
          this.cacheData();
          this.navCtrl.push("ChangeHouseInfoPage");
          // 缓存家庭人员列表，家庭资产页面需要
          this.local.write("change-jtryList", response.data);
        } else {
