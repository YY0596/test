...
import { AddChangePropertyInfoPage } from './add-change-property-info';
...
@IonicPage()
@Component({
  selector: 'page-change-property-info',
  templateUrl: 'change-property-info.html',
})
export class ChangePropertyInfoPage {
  ...
  ionViewDidLoad() {
    let cache: any = this.storage.read('change-propertyInfoCache');
    this.homeMembers = this.storage.read('change-jtryList'); //获取家庭成员信息 
    if (cache) {
  ...
  getPropertyInfo() {
    ...
    let url = "getJtzcxxOfJtxxbg.action";
    ...
    this.httpService.httpPost(8, url, body).then((response) => {
      if (response.code == 0) {
        ...
        let rep = response.data.sumJson;
        if (rep) {
          this.jtzcSumJson.jtzc_jdcls = this.jtzcSumJson.jtzc_jdcls_text = rep.jdcls.toString();
          this.jtzcSumJson.jtzc_qtzcze = this.jtzcSumJson.jtzc_qtzcze_text = rep.qtzcze.toString();
          this.jtzcSumJson.jtzc_tzlzcze = this.jtzcSumJson.jtzc_tzlzcze_text = rep.tzlzcze.toString();
          this.jtzcSumJson.jtzc_xjze = this.jtzcSumJson.jtzc_xjze_text = rep.xjze.toString();
          this.jtzcSumJson.jtzc_yhckze = this.jtzcSumJson.jtzc_yhckze_text = rep.yhckze.toString();
          this.jtzcSumJson.jtzc_zcsbrq = this.jtzcSumJson.jtzc_zcsbrq_text = rep.zcsbrq.toString();
        }
      }
  ...
  addPropertyInfo() {
    let profileModal = this.modalCtrl.create(AddChangePropertyInfoPage);
  ...
  editPropertyInfo(item) {
    let profileModal = this.modalCtrl.create(AddChangePropertyInfoPage, { item: item });
    profileModal.onDidDismiss(data => {
      if (!data) {
        return;
      }
    ...
    this.comm.showConfirm("CCB建融家园公租房","确认是否保存？", () => {
      ...
      this.httpService.httpPost(8, url, body).then(response => {
        if (response.status == 0) {
          this.getData(response.data.data);
          this.cacheData();
          this.navCtrl.push("ChangeUploadImageInfoPage");
        } else {
  ...
  cacheData() {
    ...
    this.storage.write('change-propertyInfoCache', cache);
  }
}
