...
@IonicPage()
@Component({
  selector: 'page-change-upload-image-info',
  templateUrl: 'change-upload-image-info.html',
})
export class ChangeUploadImageInfoPage {
  ...
  ionViewWillEnter() {
    //获取上传图片注意事项
    this.imgInfoNotice = this.storage.read('change-imgInfoNotice');
    this.getUploadInfo();
  }

  getUploadInfo() {
    let url = 'getSjzlList.action';
    let body = 'wwsqbh=' + this.storage.read('timestamp') + '&ywlb=233051'+
    '&zlzb='+this.storage.read("change-homeStyle")[0].key+
    '&szcq='+this.storage.read("change-szcq");
  ...
  uploadImg() {
    ...
    let body = 'wwsqbh=' + this.storage.read('timestamp') + '&bpCategoryCode=web233051&imageInfoList=' + JSON.stringify(this.imageInfoList);
    this.comm.showConfirm("CCB建融家园公租房","确定提交变更？", () => {
      this.comm.showLoading('请稍后...');
      this.httpService.httpPost(8, url, body).then(response => {
        if (response.code == 0) {
          let data = 'wwsqbh=' + this.storage.read('timestamp') + '&bpCategoryCode=web233051';
