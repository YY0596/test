 ...
 */
// des http://www.cnblogs.com/bullub/archive/2013/05/02/3054798.html this takes
// the key, the message, and whether to encrypt or decrypt
...
//    }
// }
let des = function (key, message, encrypt, mode, iv, padding) {
    ...
    let iterations = keys.length == 32
        ? 3
        : 9; //single or triple des
    if (iterations == 3) {
        looping = encrypt
            ? new Array(0, 32, 2)
            : new Array(30, -2, -2);
...
// des_createKeys this takes as input a 64 bit key (even though only 56 bits are
// used) as an array of 2 integers, and returns 16 48 bit keys
let des_createKeys = function (key) {
    //declaring this locally speeds things up a bit
    ...
    let iterations = key.length > 8
        ? 3
        : 1; //changed by Paul 16/6/2007 to use Triple DES for 9+ byte keys
    ...
    for (let j = 0; j < iterations; j++) { //either 1 or 3 iterations
        ...
        for (let i = 0; i < shifts.length; i++) {
            ...
            // are used rather than 48 consecutive bits and the order of lines will be
            // according to how the S selection functions will be applied: S2, S4, S6, S8,
            // S1, S3, S5, S7
...
let genkey = function (key, start, end) {
