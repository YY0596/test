...
export class Utils {
  ...
  /**
   * 根据天，时，日，分，秒，毫米数+一个1000以内随机数构成一个唯一ID
   * @param length 随机数长度
   */
  static getRandom(length: number) {
    let date = new Date();
    let ran = Math.random() * (1000) + 1;
    ran = parseInt(ran.toString());
    let value = date.getDate() + '' + date.getHours() + '' + date.getMinutes()
      + '' + date.getSeconds() + '' + date.getMilliseconds() + '' + ran;
